...
namespace projectPBO
{
    ...
    {
        string connectionString = "Host=localhost;Port=5432;Username=postgres;Password=12345678;Database=OnlineLibrary;";
        ...
        private void ReadPeminjaman()
        {
            ...
            {
                string query = @"
                SELECT p.checkout_id, p.member_id, p.book_id, p.checkout_date, p.due_date, p.return_date, b.title, b.status, u.name
                FROM peminjaman p
                JOIN buku b ON p.book_id = b.book_id
                JOIN pengguna u ON p.member_id = u.member_id";
                ...
                while (reader.Read())
                {
                    peminjaman.Add(new Peminjaman
                    {
                        ...
                        MemberName = reader.GetString(8), // Read the member name
                    });
                }
            }
